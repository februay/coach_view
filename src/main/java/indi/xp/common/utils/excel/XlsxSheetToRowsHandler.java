...
public class XlsxSheetToRowsHandler extends DefaultHandler {
    ...
    /*
     * 如果cell中是函数，那么 <c>下面会包含<f>和<v>两个子元素，为了只取得<v>中的值，则需要使用 _inCell和_inFormula
     * 两个参数来标识cell是否为函数 <c r="A2" t="str"> <f t="array"
    ...
    // 该函数会在 计算 <></>之间的值时进行调用
    /*
     * 比如下面的结构，characters会在计算 <v>234.0</v>中的234.0时进行调用 <c r="B1" s="1">
     * <v>234.0</v> </c>
     * 
     * 下面的结构，characters会在计算 <f t="array"
     * ref="A2">REGEXEXTRACT("hello world","hello")</f> 和 <v>hello</v>时分别进行调用
     * 计算结果为 REGEXEXTRACT("hello world","hello")hello，所以为了跳过
     * REGEXEXTRACT("hello world","hello")，需要进行判断 <c r="A2" t="str"> <f
     * t="array" ref="A2">REGEXEXTRACT("hello world","hello")</f> <v>hello</v>
     * </c>
     */
    @Override
        ...
        case NUMBER:
            ...
            if (_formatString != null) {
                ...
                if (HSSFDateUtil.isADateFormat(_formatIndex, _formatString)
                    ...
                    return DateUtils.formatDate(date, dataFormat);
                }
                if (_formatString.startsWith("reserved")) {
                    return numberString;
                }
                return formatter.formatRawCellContents(Double.parseDouble(numberString), _formatIndex, dataFormat);
            } else {

                /*
                 * 暂时注释掉，避免造成20160202被解析成2.0160202E7 modify by you.zou
                 * 2016.12.26 try { numberString =
                 * Double.parseDouble(numberString) + ""; } catch (Exception e)
        ...
        if (dataFormat != null) {
            ...
            if (dataFormatArray.length > 0) {
                ...
                newDataFormat = tmpDataFormat.replaceAll("_-", "").replace("_", "").replace("(", "").replace(")", "")
                    .replace("\"", "").replace("\\", "").replace("*", "").replace("AM/PM", "a").replace("PM/AM", "a")
                    .replace("AM", "a").replace("PM", "a").replace("AM(PM)", "a").replace("PM(AM)", "a")
                    .replace("am/pm", "a").replace("pm/am", "a").replace("am(pm)", "a").replace("pm(am)", "a")
                    .replace("a/p", "a").replace("A/P", "a")
                    // //去掉 [$-409]mmm\-yy;@ 中的 [$-409]，该结构的字符串一般会在 reserved- 的
                    // dataFormat中存在
                    .replaceAll("\\[\\$\\-.*\\]", "");
            ...
            // 处理小时，excel中hh和h在不包含am/pm是24小时制， 包含am/pm是12小时制
            if (newDataFormat.contains("hh") && !newDataFormat.contains("a")) {
                newDataFormat = newDataFormat.replace("hh", "HH");
            } else if (newDataFormat.contains("h") && !newDataFormat.contains("a")) {
                newDataFormat = newDataFormat.replace("h", "H");
            }
